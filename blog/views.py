...
def add_to_wishlist(request,pid):
	...
	posts1 = Wishlist.objects.filter(username=request.user)
	for a in posts1:
		a.product.resolve()
	paginator = Paginator(posts1,6)
	page = request.GET.get('page')
	...
	if ws:
		return render(request,'blog/my_wishlist.html',context1)
	else:
...
def search_auctions(request):
	...
	if query:
		posts = Post.objects.all().filter(is_exp=True).filter(is_verified=True)
		results=posts.filter(Q(title__icontains=query)|Q(category__icontains=query)).order_by('-date_added')
	else:
...
def search_shop(request):
	...
	if query:
		posts = Post.objects.all().filter(is_active=True).filter(is_verified=True)
		results=posts.filter(Q(title__icontains=query)|Q(category__icontains=query)).order_by('-date_added')
	else:
